...
```
  外部 OAuth2/OIDC (IdP)  ──  官网 example.com (腾讯 EdgeOne Pages/Functions/KV，登录中介)
         ▲                              │ 回环 handoff
         │ JWKS 验签                     ▼
...
- **登录**：桌面委托官网完成 OAuth（官网为 confidential client，`client_secret` 仅在 EdgeOne Pages Functions）；本地回环 handoff 拿 token；`refresh_token` 不出 EdgeOne，桌面只存 `desktop_session_id`。
...
| [`website/`](./website) | 官网（登录中介 + 营销/下载/账户中心） | React + TailwindCSS + 腾讯 EdgeOne Pages/Functions/KV |
...
- **v0** — 最小开黑回路 + 官网中介登录（登录→文字→单语音频道收发；官网下载/账户中心；Coolify + 腾讯 EdgeOne 部署）。
...
- **官网**：`cd website && npm ci && npm run build`；本地起边缘函数用 EdgeOne CLI（`npx edgeone pages dev`，具体命令以 EdgeOne CLI 为准），见 [`docs/DEV.md`](./docs/DEV.md#2-官网-website)。KV 绑定、环境变量与加密变量在 EdgeOne 控制台配置。仅 `npm run dev` 只起前端、不含函数端点。
