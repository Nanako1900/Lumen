...
│   │                            │        │                          │  │
│   │  • 调官网中介登录(回环handoff)│ binding│  • UI (频道/文字/语音/设置)│  │
...
└────────────────┼──────────────────────────────────────┼───────────────┘
                 ...
                 ▼                                        ▼
   Windows OS / 官网中介 example.com (Cloudflare)   Lumen 服务端 (经 Coolify Traefik)
```

### 1.2 职责切分原则

**铁律**：凡浏览器 webview 能做的（UI、网络 REST/WS、WebRTC、音频处理）都放前端；凡浏览器做不到或做不好的（系统浏览器拉起官网中介登录、回环 handoff、OS 凭据存储、全屏游戏全局热键、托盘、注册表、替换 exe）才放 Go 外壳。**IdP 客户端机密（client_secret/issuer/client_id/scope）全部移到官网 Cloudflare Worker，桌面不再持有**（见 [§2](#2-go-后端web-中介登录与-token-管理) 与 [`./web-design.md`](./web-design.md) §5）。

| 能力 | 归属 | 理由 | 版本 |
|------|------|------|------|
| 调官网中介登录（系统浏览器 + 127.0.0.1 回环 handoff） | **Go** | 严禁内嵌 webview 登录（钓鱼风险，调研 05 §1.1）；回环 HTTP 服务需原生网络栈；IdP 交互改由官网 Worker 完成 | v0 |
| desktop_session_id 安全存储 | **Go** | webview 无法安全访问 Windows Credential Manager；refresh_token 不出 Cloudflare，桌面只存不透明会话 ID | v0 |
| access_token 静默刷新（调官网 /api/desktop/refresh） | **Go** | 与会话生命周期耦合，集中在 Go 管理；client_secret 在 Worker，桌面只传 session ID | v0 |
...
|-----------------|------|------|------|------|
| `Login(ctx)` | — | `(LoginResult, error)` | 打开官网 `/desktop/login`、起 127.0.0.1 回环、换 token（系统浏览器 + handoff） | v0 |
| `Logout(ctx)` | — | `error` | 调官网 `/api/desktop/logout` + 清本地 `desktop_session_id` 与内存 token | v0 |
| `GetAccessToken(ctx)` | — | `(string, error)` | 取**当前有效**（必要时已向官网静默刷新）access_token | v0 |
...
├── auth/
│   ├── login.go                 # 系统浏览器 + 回环 handoff 委托官网登录 + /exchange (§2.2)
│   ├── tokenmanager.go          # 凭 desktop_session_id 调官网 /refresh 静默刷新 (§2.4)
...
> 本节描述桌面客户端如何**委托官网（[`./web-design.md`](./web-design.md) §5「Web 中介登录（桌面）」）完成登录**，而不再自行对 IdP 跑 OAuth2/PKCE。桌面只负责三件原生能力：起 `127.0.0.1` 回环监听、用系统浏览器打开官网 `/desktop/login`、把官网回传的一次性 `handoff_code` 换成 `access_token` + `desktop_session_id`。**IdP issuer/client_id/scope/client_secret 全部移到官网 Cloudflare Worker，桌面不再持有任何 IdP 客户端配置**。库版本：`github.com/pkg/browser`（开系统浏览器）、`github.com/danieljoos/wincred`（Windows Credential Manager，本项目仅 Windows）；不再依赖 `golang.org/x/oauth2`、`github.com/coreos/go-oidc/v3`。

### 2.0 身份模型与安全边界（重设计）

- **官网是 confidential OIDC client**：`client_secret` 仅存在 Cloudflare Worker 的加密环境变量，**绝不下发桌面**。官网用 Authorization Code + PKCE 对 IdP 登录，请求 scope `openid profile email offline_access`，并令 access_token 的 `aud` 含 `lumen-api`（= Go 服务端 `LUMEN_OAUTH_AUDIENCE`）。
- **桌面不再内置 IdP issuer/client_id/scope，也不再自行对 IdP 跑 PKCE**：改为「委托官网登录」（系统浏览器 + 回环 handoff）。
- **refresh_token 永不落桌面**：存在官网 Cloudflare KV `SESSIONS`。桌面只持有不透明高熵 `desktop_session_id`（存 Windows 凭据库 DPAPI），`access_token` 仅在内存。
- **Go 服务端完全不变**：仍只用 IdP 的 JWKS 本地验 `access_token`（JWT，验 `iss`/`aud`/`exp`）。账户中心不调 Lumen API，故 Go 服务端无需加 CORS。
- **官网 Worker 端点**（均在 `example.com`，详见 [`./web-design.md`](./web-design.md) §5）：`GET /desktop/login`、`GET /desktop/callback`、`POST /api/desktop/exchange`、`POST /api/desktop/refresh`、`POST /api/desktop/logout`。Cloudflare KV：`HANDOFF`（`handoff_code` → `{access_token, expires_in, refresh_token, sub, bound_challenge}`，TTL≈120s，一次性消费）、`SESSIONS`（`desktop_session_id` → `{refresh_token, sub}`）。

### 2.1 客户端配置来源

桌面客户端**不再需要 issuer/client_id/scopes**（那些已移到官网 Worker），只需指向官网与 Lumen 服务端的三个 base URL。配置仍为客户端内置默认 + 可选本地配置文件覆盖。
...
type ClientConfig struct {
	WebBaseURL string `json:"web_base_url"` // 官网中介，如 https://example.com（含 /desktop/login、/api/desktop/*）
...
> 桌面**不做** OIDC discovery、不持有 `client_secret`，也不构造任何 IdP 授权 URL。所有与 IdP 的交互（授权、换 token、刷新、撤销）都由官网 Worker 用 `client_secret` 在服务端完成。
...
1) 桌面起 127.0.0.1:<rand> 回环监听；生成 handoff_verifier(高熵) + state；
   系统浏览器打开 https://example.com/desktop/login
       ?redirect_uri=http://127.0.0.1:<port>/cb
       &state=<state>
       &challenge=S256(handoff_verifier)
2) 官网 Worker 完成对 IdP 的 OIDC（Auth Code + PKCE，access_token aud=lumen-api），
   302 回 http://127.0.0.1:<port>/cb?handoff_code=<...>&state=<...>
3) 桌面校验 state → POST https://example.com/api/desktop/exchange
...
// ExchangeResult 是官网 /api/desktop/exchange 的响应体。
...
const loginTimeout = 3 * time.Minute // 官网中介+用户操作的总超时

// Login 走「系统浏览器 + 回环 handoff」委托官网完成登录。
// webBaseURL 形如 https://example.com。
func Login(ctx context.Context, webBaseURL string) (ExchangeResult, error) {
	...
	// 3) 系统浏览器打开官网 /desktop/login（绝不内嵌 webview，防钓鱼）
	...
	// 4) 等待官网 302 回 /cb?handoff_code=&state=
	...
	// 5) 用 handoff_code + handoff_verifier 向官网换 token + desktop_session_id
...
func exchange(ctx context.Context, webBaseURL, handoffCode, verifier string) (ExchangeResult, error) {
	...
	if err != nil {
		return ExchangeResult{}, fmt.Errorf("换取令牌失败（官网中介不可用？）: %w", err)
	}
...
// s256 = BASE64URL(SHA256(s))，与官网校验 bound_challenge 的算法一致。
...
// Login [v0]：打开官网 /desktop/login、起回环、换 token。
...
func (a *App) Login(ctx context.Context) (LoginResult, error) {
	...
	// 1) 持久化 desktop_session_id（高熵不透明串，进凭据管理器；refresh_token 不出 Cloudflare）
...
// Logout [v0]：调官网 /api/desktop/logout 删服务端会话，再清本地凭据库 + 重置内存 token。
func (a *App) Logout(ctx context.Context) error {
	if err := a.tokens.Logout(a.ctx); err != nil {
		runtime.LogWarningf(a.ctx, "登出官网会话失败（仍继续清本地）: %v", err)
	}
	return nil
}
```
...
本项目仅 Windows，用 `github.com/danieljoos/wincred` 封装 Windows Credential Manager（OS 层 DPAPI 静态加密，绑定用户登录态，调研 05 §3.1 方案 B）。与旧设计的关键差异：**不再存 refresh_token**（refresh_token 永不出 Cloudflare），**只存不透明高熵 `desktop_session_id`**；`access_token` 短命，仅内存持有。
...
刷新不再走 IdP，而是用 `desktop_session_id` 调官网 `POST /api/desktop/refresh`（官网用 `client_secret` + KV 里的 refresh_token 向 IdP 刷新，IdP 轮换则官网更新 KV）。`TokenManager` 集中管理：内存缓存当前 `access_token` 及其到期时刻，临期/缺失时调官网刷新；对前端只暴露 `GetAccessToken`（拉模型）。

> **`expires_in` 防护（防刷新风暴）**：`expiresAt = now + max(expiresIn, 0) 秒`。正常按 `expiresAt − 60s` 提前刷新；当响应缺失或 `expiresIn ≤ 0` 时，视为立即过期并刷新一次，但用一个**最小刷新间隔**（如 30s，记录上次刷新时刻）兜底，避免官网/IdP 返回 `expires_in=0` 时陷入紧凑循环刷新（与 [web-design §5 `expires_in` 约定](./web-design.md#5-web-中介登录桌面)对齐）。`/api/desktop/refresh` 返回 `401 SESSION_INVALID` → 触发 `auth:expired` 重新登录。
...
// ErrSessionInvalid：官网返回 401 / SESSION_INVALID，需重新登录。
var ErrSessionInvalid = errors.New("desktop session 已失效，请重新登录")

// TokenManager 管理内存 access_token，凭 desktop_session_id 向官网刷新。线程安全。
...
// AccessToken 返回当前有效 access_token（临期或缺失则调官网刷新）。binding 入口。
...
// 官网 401/SESSION_INVALID → 触发 onExpired 并返回 ErrSessionInvalid。
func (m *TokenManager) refreshLocked(ctx context.Context) error {
	...
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		return fmt.Errorf("刷新令牌失败（官网中介不可用？）: %w", err)
	}
	defer resp.Body.Close()
...
// Logout 调官网 /api/desktop/logout 删服务端会话，再清本地凭据库与内存。
func (m *TokenManager) Logout(ctx context.Context) error {
	...
	_ = DeleteSession() // 先清本地，保证即使官网调用失败也回到未登录
	if sid == "" {
		return nil
	}
	...
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		return fmt.Errorf("登出官网会话失败: %w", err)
	}
	defer resp.Body.Close()
	return nil // 官网返回 204
}
...
> **与契约的衔接**：刷新对 Go 服务端透明——`access_token` 仍是同一个 IdP 颁发的 JWT，Go 服务端照旧用 JWKS 验 `iss`/`aud`/`exp`（契约 §2.6 规则 1）。前端收 `token:refreshed` 后，**在同一 WS 连接发 `reauth`**（不重连）。若 REST 收 `401`，前端调一次 `GetAccessToken`（内部已向官网刷新）后重试一次；若官网刷新本身返回 `401/SESSION_INVALID` → Go 触发 `auth:expired` 引导重新登录（契约 §2.6 规则 2）。

### 2.5 资料同步（用 /exchange 返回的 profile 做初始 UI）

桌面**不再直接拉 IdP userinfo**。登录时官网 `/exchange` 已返回最小 `profile:{display_name, avatar_url}`，桌面仅用它做**登录后即时 UI**；权威资料仍由服务端在 WS `auth`/`reauth` 时从 JWT claims 取并广播 `user_updated` 负责。`ProfileSync` 因此退化为本地缓存。
  ...
  private open() {
    ...
    ws.onopen = async () => {
      ...
      try {
        ...
        // desktop_session_id 为空（已登出）或官网刷新失败：不应触发重连，主动关闭并引导重新登录
...
用户在 AppSettings（§5.1 / §12.1）点「退出登录」
  ...
  ├─ 3) 调 Go Logout(ctx) binding（§1.3 / §2.4：调官网 /api/desktop/logout 删服务端会话 + DeleteSession 清 desktop_session_id + 清内存 token）
...
>
> **`onopen` 取 token 防御**：WS `onopen` 内 `await GetAccessToken()` 必须 try/catch（§7.2 已实现）；`desktop_session_id` 为空（已登出）或官网刷新返回 `SESSION_INVALID` 时不触发重连，主动关闭并 `EventsEmit("auth:expired")`，防止未处理 rejection 与「登出后 onclose 仍重连且取 token 失败」的异常态。
...
| 登录（Go `Login`） | 官网中介不可用（`/desktop/login` 打不开 / `/exchange` 网络失败） | "无法连接登录服务，请检查网络后重试" | binding 返回 error → 登录页 toast；停留登录页可重试 |
...
| 刷新（Go `refresh`） | 官网 401 / `SESSION_INVALID` | "登录已过期，请重新登录" | `EventsEmit("auth:expired")` → 清 `desktop_session_id` → 回登录页 |
| 刷新（Go `refresh`） | 官网中介不可用（网络失败 / HTTP ≠ 200/401） | "网络异常，正在重试" | `GetAccessToken` 返回 error → 前端按重连/重试处理（不强制登出） |
...
- access_token 绝不写日志/localStorage（仅内存）；`desktop_session_id` 仅存 Go 凭据管理器（DPAPI），不进前端；refresh_token 永不出官网 Cloudflare。
...
| 静默刷新（调官网 /refresh）+ 暴露 access_token | `auth/tokenmanager.go` | v0 |
